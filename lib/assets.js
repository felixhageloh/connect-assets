...
(function() {
  var BEFORE_DOT, ConnectAssets, EXPLICIT_PATH, REMOTE_PATH, Snockets, connectCache, crypto, cssCompilers, cssExts, exports, extend, fs, getExt, jsCompilers, md5Filenamer, mkdirRecursive, parse, path, stripExt, timeEq,
    __slice = [].slice;
  ...
  module.exports = exports = function(options) {
    ...
    cssCompilers.addPlugin('stylusExtends', (_ref12 = options.stylusExtends) != null ? _ref12 : options.stylusExtends = function() {
      return {};
    });
  ...
  ConnectAssets = (function() {
    ...
    ConnectAssets.prototype.compileCSS = function(route) {
      var alreadyCached, buildPath, buildPath2x, cacheFlags, css, css2x, data, dot, ext, filename, filename2x, mtime, source, sourcePath, startTime, stats, _i, _len, _ref, _ref1, _ref2, _ref3;
      if (!this.options.detectChanges && this.cachedRoutePaths[route]) {
        return this.cachedRoutePaths[route];
      }
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        try {
          ...
          if (ext === 'css') {
            ...
            if (this.options.splitHighResAssets) {
              cssCompilers[ext].resolution = '2x';
              css2x = cssCompilers[ext].compileSync(this.absPath(sourcePath), source);
            }
          ...
          if (alreadyCached && this.options.build) {
            ...
            filename2x = filename.slice(0, dot) + '@2x' + filename.slice(dot);
            this.buildFilenames[sourcePath] = filename;
            cacheFlags = {
              expires: this.options.buildsExpire,
              mtime: mtime
            ...
            if (css2x != null) {
              this.cache.set(filename2x, css2x, cacheFlags);
            }
            if (this.options.buildDir) {
              buildPath = path.join(process.cwd(), this.options.buildDir, filename);
              buildPath2x = path.join(process.cwd(), this.options.buildDir, filename2x);
              mkdirRecursive(path.dirname(buildPath), 0x1ed, function() {
                fs.writeFile(buildPath, css);
                if (css2x != null) {
                  return fs.writeFile(buildPath2x, css2x);
                }
              });
            }
            return this.cachedRoutePaths[route] = "/" + filename;
            ...
            if (css2x != null) {
              this.cache.set(stripExt(route) + '@2x.css', css2x, {
                mtime: mtime
              });
            }
            return this.cachedRoutePaths[route] = "/" + route;
